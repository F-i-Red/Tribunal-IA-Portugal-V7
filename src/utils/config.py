"""
Configuração V7 — tipagem 100% forte, validação de segurança para produção/.gov.
CORRECÇÕES:
  - Validação de API_SECRET_KEY em produção (falha hard se padrão)
  - Bloqueio de OpenRouter em GOV_MODE (soberania de dados)
  - Timeout de sessão reduzido em produção (30 min vs 60 min)
  - Suporte a mTLS para comunicação interna Ollama
  - Variável ENV e GOV_MODE para controlo de modo
"""
...
class Settings(BaseSettings):
    ...
    # Encriptação em repouso (auditoria)
    ...
    def usar_cohere_rerank(self) -> bool:
        return (
            self.rag_reranking
            and self.rag_reranker_backend == "cohere"
    ...
    def validate_and_setup(self) -> "Settings":
        ...
        # 1. Validação de segurança em produção/GOV_MODE
        if is_prod:
            if not self.api_secret_key_segura:
                raise ValueError(
                    "[SEGURANÇA] Em produção/GOV_MODE, API_SECRET_KEY deve ser alterada do padrão "
                    "e ter pelo menos 32 caracteres.\n"
                    "Gera uma chave com: python -c \"import secrets; print(secrets.token_hex(32))\""
                )

        # 2. Soberania de dados em GOV_MODE
        if self.gov_mode and self.backend == "openrouter":
            raise ValueError(
                "[SOBERANIA DE DADOS] GOV_MODE=true não é compatível com BACKEND=openrouter.\n"
                "Dados de cidadãos não podem ser enviados para servidores externos.\n"
                "Define BACKEND=ollama."
            )

        # 3. Validação OpenRouter fora de GOV_MODE
        ...
        # 4. Encriptação em repouso obrigatória em produção
        if is_prod and not self.audit_encryption_key:
            raise ValueError(
                "[SEGURANÇA] AUDIT_ENCRYPTION_KEY é obrigatória em produção/GOV_MODE.\n"
                "Gera uma chave com: python -c \"from cryptography.fernet import Fernet; "
                "print(Fernet.generate_key().decode())\""
            )

        # 5. mTLS obrigatório em GOV_MODE com Ollama remoto
        if self.gov_mode and self.backend == "ollama" and not self.ollama_url.startswith("http://localhost"):
        ...
        # 6. Em produção, reduzir timeout de token
        if is_prod and self.api_access_token_expire_minutes > 30:
            # Não falhar, mas emitir aviso via ajuste automático
            object.__setattr__(self, "api_access_token_expire_minutes", 30)

        # 7. Criar directorias necessárias
        for p in [
